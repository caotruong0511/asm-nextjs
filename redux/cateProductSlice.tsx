...
const cateProductSlice = createSlice({
    ...
    buid.addCase(getone.fulfilled,(state,{payload})=>{
        state.cateProduct= payload as CateProduct;
    }),
    buid.addCase(updateCateProduct.fulfilled,(state,{payload})=>{
        state.cateProducts=state.cateProducts=state.cateProducts.map((item)=>(item._id === payload?._id ? payload :item)) as CateProduct[]
     })
    },
})
